...
>
> **Privacy note**: the public site (`wiki/`, rendered via MkDocs) must NOT contain personal-identifying information. Owner identity stays in `MEMORY.md` (auto-loaded, not published). When editing wiki pages, do not reintroduce names, job titles, locations, or personal context — see `memory/feedback_wiki_privacy.md`.
